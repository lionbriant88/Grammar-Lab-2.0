...
def test_rule_completeness():
    ...
    # L1 已开放 kind 数量正确（M3c5: VP 包含 adverb + adverbial_clause + noun_clause）
    ...
    assert set(vp_avail) == {"adverb", "adverbial_clause", "noun_clause"}
...
def test_engine_analyze():
    ...
    # M3c5: VP 的 candidates 包含 adverb + adverbial_clause + noun_clause
    assert set(c["kind"] for c in pred.candidates) == {"adverb", "adverbial_clause", "noun_clause"}
